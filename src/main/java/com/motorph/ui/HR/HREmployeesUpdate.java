...
public class HREmployeesUpdate extends JFrame {
    ...
    public HREmployeesUpdate() {
        setTitle("MotorPH HR Employees Update");
        setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        setResizable(false);

        setContentPane(new EmployeesUpdateRootPanel());
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(new Runnable() {
            @Override
            public void run() {
                new HREmployeesUpdate().setVisible(true);
            }
        });
    }

    private static class EmployeesUpdateRootPanel extends JPanel {

        public EmployeesUpdateRootPanel() {
            ...
            ActionButton updateButton = new ActionButton();
    ...
    private static class ActionButton extends JPanel {
        ...
        public ActionButton() {
            ...
            JLabel text = label("Update", 14, Font.PLAIN, WHITE);
            text.setBounds(38, 8, 50, 18);
            add(text);
        }
    ...
    private static class FormSection extends JPanel {
        ...
        public FormSection(String title, int x, int y, FieldData[] fields) {
            ...
            for (int i = 0; i < fields.length; i++) {
                JLabel fieldLabel = label(fields[i].label, 11, Font.PLAIN, BLACK);
                fieldLabel.setBounds(0, labelY, 180, 14);
                add(fieldLabel);

                CustomTextField field = new CustomTextField(fields[i].placeholder);
